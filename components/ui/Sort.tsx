...
const Sort: FunctionComponent<SortProps> = () => {
  ...
  useClickOutside(sortRef, setOpen);
  return (
    <div
      ref={sortRef}
      onClick={toggle}
      className={clsx(
        "relative flex items-center gap-2.5 rounded-2xl bg-[#FAFAFA] px-5.5 cursor-pointer",
        open && ""
      )}
    >
      <svg
      ...
      Сортировка: <span className="text-(--orange) ">{title}</span>
      <div
        className={clsx(
          "absolute left-0 right-0 rounded-2xl py-3 bg-[#FAFAFA] top-full mt-2.5 invisible",
          open && "visible"
        )}
      >
